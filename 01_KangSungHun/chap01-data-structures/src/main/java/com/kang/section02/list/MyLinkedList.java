 ...
 * 각 노드는 데이터 + 다음 노드를 가리키는 포인터로 구성되어있다.
 */
public class MyLinkedList<T> {
  ...
  public void add(int index, T element) {
    ...
    if(index == 0){
      ...
      if(size == 0){ // 기존 저장된 요소 없다면 head, tail 모두 newNode를 가르켜야함.
        tail = newNode;
      }
    ...
    // 3-3. 삽입 위치가 중간인 경우
      ...
      for(int i = 0 ; i < index -1 ; i++) {
        current = current.next; // index - 1 번째 노드가 최종 current가 된다.
      }
